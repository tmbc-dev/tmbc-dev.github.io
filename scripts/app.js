...
async function connectToMetaMask() {
    ...
    if (typeof window.ethereum !== "undefined") {
        try {
            ...
            startBalanceUpdate(); // Start updating the balance every second
...
async function fetchUserData() {
    try {
        ...
        const truncatedBalance = (Number(balance) / 10**3).toFixed(3); // Token has 3 decimals
        
        document.getElementById("tmbc-balance").textContent = `Balance: ${truncatedBalance} TMBC`;
...
function copyReferralUrl() {
    const referralUrl = `https://tmbc.cc/app.html?r=${userAddress}`;
